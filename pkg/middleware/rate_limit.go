...
func RateLimiting(rpm uint16, options ...RateLimitingOption) echo.MiddlewareFunc {
	...
	cfg := echomw.RateLimiterConfig{
		...
		ErrorHandler: func(_ echo.Context, err error) error {
			return libErr.From(libErr.MissingIdentifier).Wrap(libErr.WithError(err))
		},
		DenyHandler: func(_ echo.Context, identifier string, err error) error {
			return libErr.From(libErr.RateLimitExceeded).Wrap(
				libErr.WithErrorf("rate limiter: denied %s: %w", identifier, err))
		},
	}
